...
class ControllerMailOrder extends Controller {
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			$order_customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");

			if ($order_customer_query->num_rows) {
				$customer_data = $order_customer_query->row;
				$data['customer_id'] = $customer_id;
				$data['customer'] = $customer_data['firstname'] . " ". $customer_data['lastname'];
			} else {
				$data['customer'] = '';
			}
